...
private:
	...
	//std::set<part_prec_3> VirtualCounterpartNormals;
...
public:
	...
	BoundaryData(part_prec_3 normal, part_prec distance, part_prec_3 Velocity) {
		...
		//VirtualCounterpartNormals.insert(normal);
	...
	bool firstBoundaryisPeriodic() { 
		return periodicBoundary[0];
		//for (auto pb : periodicBoundary) { return pb; }
	}
	part_prec_3 firstBoundaryVelocity() { 
		return VC_BoundaryVelocity[0];
		//for (auto bvel : VC_BoundaryVelocity) { return bvel; }
	}
	part_prec_3 firstDistanceVector() {
		return VirtualCounterpartNormals[0] * VC_DistanceToBoundary[0];
		//for (auto n : VirtualCounterpartNormals) {
		//	for (auto d : VC_DistanceToBoundary) {
		//		return n*d;
		//	}
		//}
	}
	part_prec_3 firstNormal() {
		return VirtualCounterpartNormals[0];
		//for (auto n : VirtualCounterpartNormals) { return n; }
	}
	part_prec_3 firstDistanceToPeriodic() { 
		return VC_DistanceToPeriodic[0];
		//for (auto d : VC_DistanceToPeriodic) { return d; }
	}
	part_prec_3 firstPeriodicBoundaryVelocity() { 
		return VC_Before_PeriodicVelocity[0];
		//for (auto bp : VC_Before_PeriodicVelocity) { return bp; }
	}
	...
	void resetAll() {
		...
		//VirtualCounterpartNormals.clear();
	...
	bool newVirtualCounterpart(part_prec_3& normal, float angle_cos) {
		...
		for (auto n: VirtualCounterpartNormals) {
			if (abs(n.x*normal.x + n.y*normal.y + n.z*normal.z) / (sqrt(pow(n.x, 2) + pow(n.y, 2) + pow(n.z, 2))*sqrt(pow(normal.x, 2) + pow(normal.y, 2) + pow(normal.z, 2))) >= angle_cos) {
