...
enum NeighboursCountingMode {
	All,        //REAL-VIRTUAL-BOUNDAR
	ONLY_REAL,  // REAL-REAL
	REAL_AND_ONE_VIRTUAL, //REAL-REAL REAL-VIRTUAL
	REAL_AND_VIRTUAL //REAL-REAL REAL-VIRTUAL, VIRTUAL-VIRTUAL
};

enum PARTICLEPAIR_NUMERATIN {
	PPN_MAIN,
	PPN_NEIGHBOUR
};
...
public:
	...
	part_prec dW(part_prec Rds, part_prec SmR) override {
		part_prec retVal;
		if (Rds >= 0 and Rds < 1) retVal = (-2.0*Rds + 3.0 / 2.0*pow(Rds, 2)) / SmR;
...
public:
	...
	part_prec dW(part_prec Rds, part_prec SmR) override {
		part_prec retVal;
		if (Rds >= 0 and Rds < 1) retVal = (-5.0*pow(3 - Rds, 4) + 30.0 * pow(2 - Rds, 4) - 75.0 * pow(1 - Rds, 4)) / SmR;
		if (Rds >= 1 and Rds < 2) retVal = (-5.0 * pow(3 - Rds, 4) + 30.0 * pow(2 - Rds, 4)) / SmR;
		if (Rds >= 2 and Rds < 3) retVal = (-5.0 * pow(3 - Rds, 4)) / SmR;
		if (Rds >= 3)             retVal = 0.0;
		return retVal;
	}
	part_prec d2W(part_prec Rds, part_prec SmR) override {
...
public:
	...
	part_prec dWd(DiffAxis axis, Particle* part_ptr) {
		...
		case Z:
			...
			break;
		case R:
			if (part_ptr == Mpart_ptr) dWd= dWd* factor;
			else if (part_ptr == NBpart_ptr) dWd= dWd* factor;
	...
	part_prec d2Wd(DiffAxis axis1, DiffAxis axis2, Particle* part_ptr) {
		...
		if ((axis1 == axis2)) {
			...
			case Z:
				...
				break;
			case R:
				if (part_ptr == Mpart_ptr)  d2Wd = mode->d2Wdr2(m_Rds, m_aveSmR)* factor;
				else if (part_ptr == NBpart_ptr)  d2Wd = mode->d2Wdr2(m_Rds, m_aveSmR)* factor;
...
public:
	...
	ParticlePair(Particle* mpart, Particle* nbpart, DIMENSIONS dim, cd_prec &mindistance)
		...
		case(All):
			...
			nbpart->addNeighbour();
			break;
		case(REAL_AND_VIRTUAL):
			if ((mpart->m_type == VIRTUAL) and (nbpart->m_type == VIRTUAL)) {
				mpart->addNeighbour();
				nbpart->addNeighbour();
			}
		case(REAL_AND_ONE_VIRTUAL):
			if (((mpart->m_type == REAL) and (nbpart->m_type == VIRTUAL))or ((nbpart->m_type == REAL) and (mpart->m_type == VIRTUAL))) {
				mpart->addNeighbour();
				nbpart->addNeighbour();
			}
		case(ONLY_REAL):
			if((mpart->m_type == REAL) and (nbpart->m_type == REAL)) {
				mpart->addNeighbour();
				nbpart->addNeighbour();
			}
			break;
